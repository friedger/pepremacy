;; Styx v4 Pool structure to track sBTC liquidity (single pool per contract)
...
(define-constant OPERATOR_STYX 'SP3MZYWF2JWWYSP5NEH9Z8Y46D3HZVY3CZD5RGVHD) ;; in beta 'SMH8FRN30ERW1SX26NJTJCKTDR3H27NRJ6W75WQE 
        ...
        (match (get out (unwrap! (get-out-value wtx btc-receiver) ERR_NATIVE_FAILURE))
          out (if (>= (get value out) MIN_SATS)
            ...
            (print {
              ...
              (match (as-contract (contract-call? .pool swap-a-to-b sbtc-amount-to-user min-pepe-out))
                delta (ok delta)
                error (begin
